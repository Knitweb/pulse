...
| [09-quanta-series.md](09-quanta-series.md) | **The Quanta Series: qua-core to the Browser-Native Node.** Reconstructed capture of the owner's "qua-*" design conversation (the lost `pulse-ar-series-FULL.bundle`): the eleven-step module ladder (core → identity → event field → economy → assets → AR → game → quantum → web UI → runtime), the ten-gap analysis with the minimal vertical demo, the **browser-as-spider** pivot (WebGPU · WebRTC · WebXR · IndexedDB · Service Workers), the **≤49% GPU covenant**, and the full crosswalk + non-negotiable reconciliation onto the Knitweb layers. |
